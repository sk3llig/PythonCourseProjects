...
def search_word(filename: str, word: str) -> int:
    f=load_file(filename).replace('\n','')
    s=[c for c in set(f) if not c.isalpha()]
    f=f.lower()
    for c in f:
        if c in s: f=f.replace(c, ' ')
    f=f.split()
    return f.count(word)
...
def load_table(filename: str) -> List[Dict[str, str]]:
    ...
    with open(filename) as fr:
        for i,line in enumerate(fr):
            if i==0: lk+=(line.replace('\n','')).split()
            else: lv.append((line.replace('\n','').split()))
    agenda=[]
    for l in lv:
        agenda.append({k:v for k,v in zip(lk,l)})
    return agenda
...
def load_csv(filename: str, empties: bool) -> List[Dict[str, Union[int, str, None]]]:
    ...
    for l in lv:
        ...
        for k,v in zip(lk,l):
            ...
            if v=='' :
                v=None
                if (empties!=True and empties!=None): continue
...
def column_stats(filename: str) -> Dict[str, Tuple[str, int]]:
    agenda=load_csv(filename, empties=False)
    new_a=[]
    lk=[]
    lv=[]
    i=0
